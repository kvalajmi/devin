...
import { useInvestorWithdrawals } from './hooks/useInvestorWithdrawals'
import AddWithdrawalForm from './AddWithdrawalForm'

interface InvestorWithdrawalsTableProps {
  investorId: number
  investorName: string
}
 ...
 * مسؤولية واحدة: عرض جدول مسحوبات المستثمر مع إمكانية السحب
 */
const InvestorWithdrawalsTable: React.FC<InvestorWithdrawalsTableProps> = ({
  investorId,
  investorName
}) => {
  const [showAddForm, setShowAddForm] = useState(false)
  const { records, isLoading, error, addRecord, getTotalWithdrawals } = useInvestorWithdrawals(investorId)
      ...
      <div className="bg-gradient-to-r from-red-600 to-red-700 p-4 text-white">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold flex items-center">
            ...
            كشف مسحوبات المستثمر
          </h3>
          <div className="flex items-center space-x-2 space-x-reverse">
            <div className="text-sm bg-red-500 bg-opacity-30 px-3 py-1 rounded-full">
              الإجمالي: {formatCurrency(getTotalWithdrawals())}
            </div>
            <button
              onClick={() => setShowAddForm(!showAddForm)}
              className="bg-red-500 hover:bg-red-400 text-white px-4 py-2 rounded-lg transition-colors text-sm font-medium"
            >
          ...
          <table className="w-full border-collapse">
              ...
              {records.map((record, index) => (
                <tr key={record.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 border border-gray-300">
                    {record.notes || 'سحب من ' + investorName}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-red-600 border border-gray-300">
            ...
            <tfoot className="bg-red-50">
              <tr>
                ...
                <td className="px-6 py-3 text-sm font-bold text-red-600 border border-gray-300">
...
export default InvestorWithdrawalsTable
